const DBHelper = require('./db-sqlserver-helper');
...
class Biz {
    ...
    static async getBiz(bizName) {
        ...
        let redis = null;
        try {
            redis = await redisHelper();
            if (await redis.exists(bizName)) {
                for (let i = 0; i < 4; i++) {
                    biz_arr.push(JSON.parse(await redis.lindex(bizName, i)));
                }
            }
            IsUseCache = true;
        ...
        if (biz_arr.length < 4) {
            ...
            let ps = [{ name: 'BizName', type: TYPES.VarChar, value: bizName }];
            ...
            if (IsUseCache) {
                ...
                for (let i = 3; i >= 0; i--) {
                    await redis.lpush(bizName, JSON.stringify(biz_arr[i]));
                }
                redis.quit();
            }
        }
...
class BizQuery extends Biz {
    ...
    async execute(ps) {
        ...
        let dbhelper = new DBHelper();
        try {
            let runPs = p._getSqlRunParamaters(ps);
            let page = ps.page ? ps.page : 1;
            let pagesize = ps.pagesize ? ps.pagesize : 10;
            ...
            //执行统计查询
            let isHasCountTag = /--\[Pagenation[\w\W]*--\[Columns\][\w\W]*--\[\/Columns\][\w\W]*--\[\/Pagenation\]/g.test(p.sqls[0].sql);
            try {
    ...
    async getcolumnMetadata() {
        ...
        try {
            ...
            if (_.isArray(p.params) && p.params.length > 0) {
                ...
                ps.push({ name: 'Operator', type: TYPES.VarChar, value: null });
            }
            for (let sql of p.sqls) {
                let runSql = p._getRunSql(sql.sql);
                let arr = [];
                let columnMeta = await dbhelper.getColumnMetadata(runSql, ps);
                for (let col of columnMeta) {
                    arr.push({
            ...
            throw new Error('获取字段失败：' + ex.message);
        }
        return rs;
    }
    _getSqlRunParamaters(ps) {
        let p = this;
        let psMap = p._getSqlParamaters(ps);
        /* let psArr = [];
    ...
    _pagingGenerator(sql, sortName, sortOrder = 'asc') {
        let beginMatch = sql.match(/--\[Pagenation:[\w\s\,]*\]/g);
        let endMatch = sql.match(/--\[\/Pagenation\]/g);
    ...
    _countGenerator(sql) {
        let columnsMatch = sql.match(/--\[Columns\][\w\W]*--\[\/Columns\]/g);
        let pagenationMatch = sql.match(/--\[Pagenation:[\w\s\,]*\]/g);
...
class BizOperation extends Biz {
    ...
    async execute(ps) {
        ...
        try {
            ...
            let outputPs = p._findOutputParamaters();
            let outputPsMap = new Map();
            let psMap = p._getSqlParamaters(ps);
            let checkRunPs = p._getSqlRunParamaters(psMap);
    ...
    _getSqlRunParamaters(psMap, loopIndex = 0, loopField) {
        ...
        if (p.params && p.params.length > 0) {
            ...
            if (loopField && loopField.trim().length > 0) {
                let tempStr = '[' + loopIndex + '][' + loopField + ']';
    ...
    async _executeCheck(dbhelper, check, ps) {
        try {
            let rs = await dbhelper.query(this._getRunSql(check.sql), ps);
        }
        catch (ex) {
            throw new Error('业务检查Sql错误：' + ex.message);
        }
        if (rs.length == 0) {
            let eMsg = check.id + '.*.' + check.cue;
            for (let o of ps) {
    ...
    _checkFieldIsLoop(checkField) {
        let loopParams = this._findLoopParamaters();
...
class BizManage extends Biz {
    static async saveBiz(biz) {
        let dbhelper = new DBHelper();
        let sql = this.exportBiz(biz);
        await dbhelper.executeSqlTran(sql);
    }
    static async addBiz(bizName, isSys, type) {
        let dbhelper = new DBHelper();
        let sql = 'Insert into FRM_Biz(s_BizName,n_Type,b_IsSys) values (@BizName,@Type,@IsSys)';
        let ps = [
...
*****************************************************/
    ...
    static async getBizs(bizType = 0) {
        let dbhelper = new DBHelper();
        let sql = `select distinct BizName=s_BizName,IsSys=isnull(b_IsSys,0), BizType =n_Type from FRM_Biz 
        where n_Type=case when @BizType=0 then n_Type else @BizType end`;
        let ps = [{ name: 'BizType', type: TYPES.Int, value: bizType }];
        let rs = await dbhelper.query(sql, ps);
        dbhelper.close();
        return rs;
    }
}
