...
*****************************************************/
    ...
    static async getBizs(bizType = 0) {
        var dbhelper = new DBHelper();
        var sql = `select distinct BizName=s_BizName,IsSys=isnull(b_IsSys,0), BizType =n_Type from FRM_Biz 
        where n_Type=case when @BizType=0 then n_Type else @BizType end`;
        var ps = [{ name: 'BizType', type: TYPES.Int, value: bizType }];
        var rs = await dbhelper.query(sql, ps);
        dbhelper.close();
        return rs;
    }
}
